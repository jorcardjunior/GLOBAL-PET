...
│   │   ├── EmployeeModal.tsx        # Troca rápida de usuário ativo e autenticação PIN
│   │   ├── EquipeScreen.tsx         # Painel administrativo de gestão de equipe e listagem de colaboradores
│   │   ├── EmployeeFormModal.tsx    # Modal de cadastro e edição de funcionário com 3 etapas
│   │   ├── EmployeeManagementModal.tsx # Modal de RH para gestão de equipe a partir de qualquer ponto
...
| `employees` | `EmployeeUser[]` | Colaboradores cadastrados com cargos, turnos, limites e PINs exclusivos. |
| `currentUser` | `EmployeeUser` | Usuário atualmente operando a sessão ativa. |
...
## 👥 Módulo de Gestão da Equipe, Cadastro de Funcionários & Categorias

### 1. Visão Geral e Arquitetura do Módulo
O módulo de recursos humanos e controle hierárquico da agropecuária permite cadastrar, editar, desativar e excluir colaboradores com regras rigorosas de segurança, alçadas financeiras e conformidade CLT.

O sistema divide os colaboradores em **7 Categorias / Cargos Profissionais**, cada qual com seu preset automático de privilégios:
1. **Proprietário / Dono (`dono`)**: Acesso irrestrito a todos os dados fiscais, DRE, alteração de CNPJ/PIX, monitoramento remoto 24h e trava de emergência.
2. **Gerente Geral da Loja (`gerente`)**: Gestão de caixa diário, autorização de descontos especiais, concessão de passes remotos temporários e cadastro de colaboradores.
3. **Operador de Caixa PDV (`operador`)**: Vendas rápidas no balcão, leitura de código de barras e recebimentos. Bloqueado para DRE e alteração de parâmetros mestres.
4. **Vendedor Balconista / Ração & Agro (`vendedor`)**: Atendimento aos produtores rurais, pesagem na balança, consulta técnica de defensivos e rações a granel.
5. **Esteticista Animal / Tosador (`tosador`)**: Operação da fila do Banho & Tosa, marcação de serviços concluídos e envio de alertas por WhatsApp aos tutores.
6. **Médico Veterinário / RT (`veterinario`)**: Consultas clínicas, aplicação de vacinas, prescrição de medicamentos controlados e acompanhamento de prontuários.
7. **Administrador TI / Corporativo (`admin`)**: Manutenção de servidores, backup de dados, auditoria de logs e segurança da rede.

### 2. O Processo de Cadastro em 3 Etapas (`EmployeeFormModal.tsx`)
Ao clicar em **"Cadastrar Novo Funcionário"**, o modal inteligente organiza o cadastro em três eixos:
- **Etapa 1: Dados & Cargo**:
  - Nome completo, CPF (opcional para folha), telefone/WhatsApp e e-mail corporativo.
  - Seleção visual de Cargo: ao selecionar um cargo, o sistema preenche automaticamente a descrição, a cor do crachá e a matriz de permissões recomendada.
  - Gerador de PIN Numérico de 4 dígitos (com validação anti-colisão para que nenhum operador tenha o mesmo PIN).
  - Galeria de Avatares com fotos reais organizadas por função ou campo para inserir URL customizada.
- **Etapa 2: Turno & Regras Operacionais**:
  - Horário de Entrada e Saída do Expediente CLT (ex: 08:00 às 18:00).
  - Dias da semana trabalhados (Segunda a Sábado, etc.).
  - Limite Máximo de Desconto Autônomo no PDV (0%, 3%, 5%, 10%, 15%, 20% ou 100%). Descontos acima do teto exigem PIN de supervisor.
  - Chave de Acesso Remoto Permanente: autoriza ou não o operador a se conectar de casa/4G sem requerer aprovação a cada sessão.
  - Status do Colaborador: Ativo / Inativo.
- **Etapa 3: Matriz de Permissões Granulares**:
  - Controle individual de cada privilégio (Acesso ao DRE, Fechamento de Caixa, Visualização de Custos de Fábrica, Emissão de Pedidos de Atacado, etc.), com botão para **"Restaurar Padrão do Cargo"**.

### 3. Onde o Módulo Está Integrado
- **Aba Nativa em Gestão (`GestaoScreen.tsx`)**: O gestor encontra o botão **"Equipe & Funcionários"** ao lado do Painel Financeiro e Caixa Diário, visualizando cards de todos os colaboradores, horários, atalhos de edição rápida, desativação e botão de assunção de sessão.
- **Atalho no Modal de Troca de Operador (`EmployeeModal.tsx`)**: O botão **"Gerenciar Equipe / Cadastrar"** no rodapé permite saltar direto para a administração da folha.
- **Modal Universal (`EmployeeManagementModal.tsx`)**: Permite invocar a tela de equipe a partir de qualquer ponto do sistema via `openEmployeeManagement()`.

---

## 📜 Histórico de Fases e Etapas de Desenvolvimento

Para facilitar a continuidade por qualquer Inteligência Artificial ou desenvolvedor:

- **Fase 1 - Frente de Caixa (PDV) & Balcão Rápido**:
  - Implementação do carrinho de compras, cálculo de quilos fracionados na balança, formas de pagamento (PIX, Dinheiro, Cartão, Fiado) e emissão de cupom térmico 80mm.
- **Fase 2 - Tambores de Ração a Granel & Pesagem**:
  - Medidor de nível dos tambores, rendimento de sacas de 15kg/20kg, alerta de ponto de pedido e cálculo de margem agregada de desdobro.
- **Fase 3 - Centro de Estética (Banho & Tosa) & WhatsApp**:
  - Fila kanban de pets (Aguardando, Na Banheira/Mesa, Pronto), módulo de Táxi Dog e gerador automático de mensagens de WhatsApp com emojis e tags dinâmicas.
- **Fase 4 - Caderninho de Fiado Comunitário & Boletos**:
  - Limite de crédito por cliente, histórico de compras a prazo, recebimentos parciais e conciliação de contas a pagar.
- **Fase 5 - Fechamento de Caixa & DRE Gerencial (Power BI Style)**:
  - Faturamento diário, sangrias, cálculo do CMV, ponto de equilíbrio, lucro bruto e despesas operacionais.
- **Fase 6 - SEFAZ Monitor & Leitor de Notas Fiscais com IA (Gemini)**:
  - Monitor de notas emitidas contra o CNPJ (manifestação do destinatário) e leitor de PDF de NF-e via backend Express + `@google/genai` (modelo `gemini-2.5-flash`).
- **Fase 7 - Gestão Remota & Blindagem Anti-Fraude CLT**:
  - Diferenciação entre rede local física e acesso remoto, concessão de passes com prazo de expiração, trava de emergência para donos e logs de auditoria de segurança.
- **Fase 8 - Responsividade Mobile de Avisos & Centralização**:
  - Correção dos cartões de avisos operacionais no celular para centralização perfeita e adição de botão [X] de fechamento intuitivo.
- **Fase 9 - Módulo Completo de Gestão de Equipe & Categorias CLT**:
  - Estruturação dos 7 cargos da agropecuária, tela completa de gerenciamento da folha (`EquipeScreen.tsx`), modal de cadastro em 3 etapas (`EmployeeFormModal.tsx`), validações de PIN exclusivo, persistência em `localStorage` e integração com a tela de Gestão.
- **Fase 10 - Blindagem de Segurança da Aplicação (AppSec) & Conformidade LGPD**:
  - Implementação de defesa em profundidade: Hashing PBKDF2 com Salt criptográfico único para PINs, proteção ativa contra força bruta com lockout de 15 minutos após 5 erros, rate limiting em múltiplos níveis (global e rotas sensíveis de autenticação), bloqueio automático de terminal por inatividade (5 minutos) ou atalho (`Ctrl + L`), mascaramento de dados no balcão contra bisbilhoteiros (*shoulder surfing*), anonimização cadastral irreversível com preservação fiscal (LGPD Art. 18 e Art. 16, I), validação de upload com verificação de cabeçalho binário (*magic bytes*) e headers HTTP seguros via Helmet.

---

### 🛡️ 10. Arquitetura de Segurança da Informação (AppSec) & LGPD

O sistema foi rigorosamente auditado e blindado segundo as melhores práticas do **OWASP Top 10** e da **Lei Geral de Proteção de Dados (Lei 13.709/2018 - LGPD)**:

| Dimensão de Segurança | Ameaça Mitigada | Implementação Técnica no Código |
| :--- | :--- | :--- |
| **Autenticação & Hash de PINs** | Vazamento de banco de dados e senhas em texto puro | Uso de `crypto.pbkdf2Sync` com 100.000 iterações de SHA-512 e Salt criptográfico randômico de 16 bytes. Os hashes gerados são irreversíveis e resistentes a tabelas rainbow. |
| **Proteção contra Força Bruta** | Ataques automatizados de dicionário em PINs numéricos de 4 a 6 dígitos | Memória do backend monitora tentativas por funcionário/IP. Na 5ª falha consecutiva, o terminal entra em estado de bloqueio compulsório (*lockout*) por 15 minutos, liberável apenas por credencial mestre do Dono ou tempo. |
| **Rate Limiting em Camadas** | Negação de Serviço (DoS) e varredura de endpoints | Middleware `express-rate-limit` configurado em duas camadas: limite geral de 300 requisições/15min e limite restrito de 15 requisições/15min para endpoints de autenticação (`/api/auth/*`). |
| **Bloqueio de Terminal Ocioso** | Acesso indevido ao caixa enquanto o operador vai ao estoque ou almoço | Listener de inatividade monitora eventos de mouse, teclado e toque. Após 5 minutos sem interação, aciona a tela de bloqueio do terminal (`PinLockScreenModal`). Também acionável via botão no cabeçalho ou atalho universal `Ctrl + L`. |
| **Privacidade no Balcão (Shoulder Surfing)** | Clientes da fila ou terceiros visualizando CPF e dados pessoais na tela | Botão de alternância de privacidade LGPD no cabeçalho e na aba de clientes. Quando ativo, aplica máscara em telefones `(11) 9****-1234` e CPFs `123.***.***-00` em todos os cartões e relatórios públicos de atendimento. |
| **Direito do Titular (LGPD Art. 18)** | Solicitação de exclusão/anonimização de dados cadastrais de clientes | Endpoint seguro `POST /api/lgpd/anonymize-client` substitui CPF, notas internas, endereço e telefone por hashes irreversíveis, mantendo o histórico de vendas para cumprimento de obrigação legal/fiscal (LGPD Art. 16, I). |
| **Hardening de Cabeçalhos HTTP** | Injeção de MIME-type, Clickjacking e XSS | Pacote `helmet` integrado no `server.ts` aplicando `X-Content-Type-Options: nosniff`, `X-DNS-Prefetch-Control`, `Referrer-Policy` e HSTS estrito. |
| **Validação Rigorosa de Uploads** | Upload de executáveis camuflados ou scripts maliciosos (RCE) | Verificação de extensão combinada com validação de *Magic Bytes* (assinatura binária real do arquivo: `%PDF-` para PDFs e `<?xml` para XMLs da SEFAZ). |
| **Auditoria & Rastreabilidade** | Ações de supervisor sem histórico | Trilha de logs de segurança imutável gravando IP, dispositivo, horário, operador e ação executada (descontos, desativação de travas e anonimizações). |
